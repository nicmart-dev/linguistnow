...
import { SUPPORTED_CURRENCIES } from "@linguistnow/shared";
import type {
...
export const searchLinguists = async (
  ...
  try {
    ...
    // Validate displayCurrency against supported currencies
    if (
      ...
      res.status(400).json({
        ...
        totalPages: 0,
        filters: req.query,
        error: `Unsupported currency: ${query.displayCurrency}. Supported: ${SUPPORTED_CURRENCIES.join(", ")}`,
      });
      return;
    }
    ...
    const linguists = await Promise.all(
      records.map(async (record): Promise<LinguistWithAvailability | null> => {
        ...
        const hourlyRate = fields[AIRTABLE_FIELDS.HOURLY_RATE] as
          | number
          | undefined;
        const currency = fields[AIRTABLE_FIELDS.CURRENCY] as string | undefined;
